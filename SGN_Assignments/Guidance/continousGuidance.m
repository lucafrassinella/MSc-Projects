...
data.t0 = 0;
data.rho0 = rho0/DU;
data.odeOpt = odeset('RelTol', 1e-12, 'AbsTol', 1e-12);
...
%% 4:
clc; clearvars -except data x0 xf;
N_iter = 5;
[sol, err, gradErr] = computeOrbit(N_iter, data, x0, xf);

%% Save solutions:
solutions.pt4.ll0 = sol(1:7);
solutions.pt4.tf = sol(end);
solutions.pt4.errPos = norm(err(1:3).*data.DU);
solutions.pt4.errVel = norm(err(4:6).*data.DU/data.TU);


fprintf('Norm of position error: %.6f km\n', solutions.pt4.errPos);
fprintf('Norm of velocity error: %.6f km/s\n', solutions.pt4.errVel);
...
state0 = [x0; ll0];
[tt, xx] = propagate(state0, data.t0, tf, data);
...
%% PT.5 - Lower Thrust Level solution through numerical continuation:
...
for i = 2 : length(TmaxVec)
   ...
   'SpecifyObjectiveGradient', true, 'FunctionTolerance', 1e-06, 'Algorithm', 'trust-region-dogleg');
    optimizationFun = @(guess) optimization(x0, guess, data, xf);
...
% Save solutions
solutions.pt5.ll0 = sol(1:7);
solutions.pt5.tf = sol(end);
solutions.pt5.errPos = norm(err(1:3).*data.DU);
solutions.pt5.errVel = norm(err(4:6).*data.DU/data.TU);

fprintf('Norm of position error: %.6f km\n', solutions.pt5.errPos);
fprintf('Norm of velocity error: %.6f km/s\n', solutions.pt5.errVel);
...
for i = 1 : length(tt)
    ...
    % H = q_f + dot(ll_f(1:3), vv_f) - mu/r_f^3 * dot(ll_f(4:6), rr_f) - Tmax/m_f * lv_f - ll_f(7) * Tmax/(Isp * g0);
...
function [tt, xx, STM] = propagate(state0, t0, tf, data)
...
[tt, xx] = ode113(fun, [t0 tf], xv0, data.odeOpt);
...
function [residuals, grad] = optimization(x0, guess, data, xf)
...
ll0 = guess(1:7);
state0 = [x0; ll0];
[tt, xx, STM] = propagate(state0, t0, tf, data);
...
% grad = objectiveGradients(STM)
statefinal = xx(end, 1:14)';
grad = objectiveJacobian(tfinal, statefinal, STM, data);
end

function [sol, err, gradErr] = computeOrbit(N_iter, data, x0, xf)
...
STM_lmlm = STM(14, 14);
STM_lmll = [STM(14, 8:10) STM(14, 11:13) STM(14, 14)];
