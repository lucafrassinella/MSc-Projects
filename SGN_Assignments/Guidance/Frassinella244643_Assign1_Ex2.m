...
% Solve optimization problem:
[varsOptSS_B, DV_SS_B] = fmincon(J, vars0, [], [], [], [], [], [], nonlincon, options);
% Check constraints compliance of the solution:
...
function [tt, xx, STM] = propagator(xx0, ti, tf, constants, computeSTM)

% Switch to avoid computations on STM if not necessary:
if nargin < 5
    computeSTM = false; % Avoid STM computations if not stated otherwise
end

if computeSTM
    % initialize STM:
    STM0 = eye(4); 
    % Append to initial state vector:
    xx0 = [xx0; STM0(:)];
end

% Integrate:
optset = odeset('RelTol', 1e-12, 'AbsTol', 1e-12);
[tt, xxv] = ode78(@(t,xx) PBRFBP(t, xx, constants, computeSTM), [ti tf], xx0, optset);
