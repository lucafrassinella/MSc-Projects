...
[xx, tt] = propagate3D(0, correctedState, tf, mu, true);
...
for i = 1 : length(cvec)
    ...
    [xx, tt] = propagate3D(0, corrState, tfNew, mu, true);
...
if strcmpi(pointType, 'triangular')
    ...
    % triangularFnctn = @(Lxy) [Lxy(1) - (1-mu)*(Lxy(1)+mu)./((Lxy(1)+mu).^2 + Lxy(2).^2).^(3/2) - mu*(Lxy(1)-(1-mu))./((Lxy(1)-(1-mu)).^2 + Lxy(2).^2).^(3/2);
    %                           Lxy(2) - (1-mu)*Lxy(2)./((Lxy(1)+mu).^2 + Lxy(2).^2).^(3/2) - mu*Lxy(2)./((Lxy(1)-(1-mu)).^2 + Lxy(2).^2).^(3/2)];
...
function [xx, tt] = propagate3D(t0, xx0, tf, mu, evtFlag)
...
while (abs(err(1))>tol || abs(err(2))>tol || abs(err(3))>tol || abs(err(4))>tol) && iter < Nmax
    ...
    [xx, tt] = propagate3D(0, newState, newGuess(4), mu, true);
    xf = xx(end, :);
    te = tt(end);
