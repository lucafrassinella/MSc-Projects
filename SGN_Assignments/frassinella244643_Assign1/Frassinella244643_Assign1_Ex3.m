...
results.ex4.errVel = norm(err(4:6).*data.DU/data.TU)*1e03;
...
for i = 2 : length(TmaxVec)
   ...
   'SpecifyObjectiveGradient', true, 'FunctionTolerance', 1e-10, 'StepTolerance', 1e-10, 'OptimalityTolerance', 1e-10, 'Algorithm','trust-region-dogleg');
...
results.ex5.errVel = norm(err(4:6).*data.DU/data.TU)*1e03;
...
%% Clear Workspace:
clearvars -except data results settings

%% FUNCTIONS

function [dx] = TPBVP(~, x, data)
% ----------------------------------------------------------------------- %
% TPBVP - Function to compute the RHS of the Euler Lagrange equations of 
% motion for the Two-Point Boundary Value Problem with State Transition Matrix 
% (STM) computation.
%
% Inputs:
...
%               - [210,1] 
%   - data - structure containing data of the problem:
%               - data.mu: adimensionalized gravitational parameter
%               - data.g0: adimensionalized gravity acceleration
%               - data.Isp: adimensionalized specific impulse
%               - data.Tmax: adimensionalized maximum thrust level
%               - data.rho0: adimensionalied reference radius
%
% Outputs:
%   dx      - Derivative of the state vector (with STM) [210, 1]
% ----------------------------------------------------------------------- % 
...
% Derivative of debris density function:
...
% Compute derivative of the STM with variational approach:
...
% ----------------------------------------------------------------------- %
% TPBVP - Function to compute the RHS of the Euler Lagrange equations of 
% motion for the Two-Point Boundary Value Problem (without STM computation)
%
% Inputs:
%   - t: Current time (here, unitilized) [1, 1]
%   - xx: Current state vector:
...
%
% Outputs:
%   dx      - Derivative of the state vector (with STM) [14, 1]
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
% propagate - Function to propagate the dynamics using the Euler Lagrange
% equations model 
%
% Inputs:
%   - state0: Initial state vector [14,1] 
%   - t0: Initial time [1, 1]
%   - tf: Initial time [1, 1]
%   - data - structure containing data of the problem:
%               - data.mu: adimensionalized gravitational parameter
...
%
% Outputs:
%   - xx: integrated discretized states [n, 14] --> i-th row is the state at
%   i-th time
%   - tt: vector of discretized times [n, 1]
%   - STM: state-transition matrix at final time [14, 14]
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
% optimization - Function to compute the residuals and gradients of the
% objective function for the zero-finding problem
%
% Inputs:
%   - x0: Initial state vector [14,1] 
%   - guess: Initial guess for lambda0 and tf [8, 1]
%   - data - structure containing data of the problem:
%               - data.mu: adimensionalized gravitational parameter
%               - data.g0: adimensionalized gravity acceleration
%               - data.Isp: adimensionalized specific impulse
%               - data.Tmax: adimensionalized maximum thrust level
%               - data.rho0: adimensionalied reference radius
%   - xf: target final state [6, 1]
%   - settings: structure containing settings for ode solver (settings.odeOpt)
%
% Outputs:
%   - residuals: vector of function residuals [8, 1]
%   - grad: matrix of gradients of objective function [8, 8]
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
% computeOrbit - Function for the zero-finding problem to compute the
% corrected lambda0 and final time tf
%
% Inputs:
...
%
% Outputs:
%   - sol: vector of optimal solution [8, 1]
%   - err: vector of errors wrt target state 
%   - gradErr: matrix of gradients of objective function [8, 8]
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
% rot2NTW - Function to rotate the primer vector Earth Centered Inertial 
% frame to NTW frame
%
% Inputs:
%   - xxECI: Matrix of state vectors in the ECI frame [n, 6] 
%   - alphaECI: Matrix of primer vectors in the ECI frame [n, 3]
%
% Outputs:
%   - alphaNTW: Matrix of primer vectors in the NTW frame [n, 3]
% ----------------------------------------------------------------------- % 
...
% ----------------------------------------------------------------------- %
% jacobianDyn - function to evaluate the jacobian of the Dynamics to compute 
% the STM through variational approach
%
% Inputs: 
%   - state: current state vector [14, 1]
%   - data - structure containing data of the problem:
%               - data.mu: adimensionalized gravitational parameter
%               - data.g0: adimensionalized gravity acceleration
%               - data.Isp: adimensionalized specific impulse
...
%
% Outputs:
%   - Jacobian: Jacobian of the dynamics [14, 14]
% ----------------------------------------------------------------------- %
...
% ----------------------------------------------------------------------- %
% objectiveJacobian - function to compute the gradients of the objective
% function of the zero-finding problem
%
% Inputs:
%   - tfinal: final time [1, 1] 
%   - stateFinal: final state at final time [14, 1]
%   - STM: state transition matrix at final time [14, 14]
%   - data - structure containing data of the problem:
%               - data.mu: adimensionalized gravitational parameter
%               - data.g0: adimensionalized gravity acceleration
...
% Compute Dynamics at final time:
...
% Assemble derivatives of the objective funcction wrt optimization variables:
