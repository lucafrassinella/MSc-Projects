...
%%%% Plots:
% Evolution of Azimuth and Elevation from ground station:
plotLocalCoords(lc.kourou.TBP, kourou, lc.kourou.et_vec)
plotLocalCoords(lc.troll.TBP, troll, lc.troll.et_vec)
plotLocalCoords(lc.svalbard.TBP, svalbard, lc.svalbard.et_vec)

% Visible Azimuth and Elevation within the visibility windows:
figure();
plot(visibleCoords.kourou_TBP.azimuth, visibleCoords.kourou_TBP.elevation, 'o')
axis([-180, 180, 0, 90])
xlabel('Azimuth [deg]')
ylabel('Elevation [deg]')
title('KOUROU Visible Azimuth and Elevation')

figure();
plot(visibleCoords.troll_TBP.azimuth, visibleCoords.troll_TBP.elevation, 'o')
axis([-180, 180, 0, 90])
xlabel('Azimuth [deg]')
ylabel('Elevation [deg]')
title('TROLL Visible Azimuth and Elevation')

figure();
plot(visibleCoords.svalbard_TBP.azimuth, visibleCoords.svalbard_TBP.elevation, 'o')
axis([-180, 180, 0, 90])
xlabel('Azimuth [deg]')
ylabel('Elevation [deg]')
title('SVALBARD Visible Azimuth and Elevation')
...
% visibility - Function to extract visibility windows and corresponding 
...
function plotLocalCoords(lc_station, station, tspan)
...
plot(tspan / cspice_spd(), azimuth, 'DisplayName', 'SMOS Azimuth')
ylabel('Azimuth [deg]','Color','k') 
ax = gca;
ax.YColor = 'k';
yyaxis right % Right axis for elevation
plot(tspan / cspice_spd(), elevation, 'DisplayName', 'SMOS Elevation')
grid on
